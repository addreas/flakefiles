{ lib, pkgs, config, modulesPath, ... }:
{
  users.users.addem = {
    isNormalUser = true;
    uid = 1000;
    extraGroups = [ "wheel" "dialout" "power" "networkmanager" "docker" "wireshark" ];

    hashedPasswordFile = "/etc/shadow.d/addem";

    openssh.authorizedKeys.keys = [
      "ssh-rsa  addem1234@gmail.com"
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAII4ASGDaGjOmmbjIHxQbVwRrd+oBwFp/R+4nxcR8EtP0 andreas@addem.se"
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJ9Vo7cki3sDDlWWyNi59wTFRa9rzkVf38cY/r86v5yj addem@nixos"
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIJAp7DrhFRPktGiPsSHrISBKQ2tl63s1Z3hj5DlZpJk6 lenny"
    ];
  };

  environment.systemPackages = with pkgs; [
    vim
    helix
    wget
    curl
    fzf
    jq
    yq
    dig
    git
    git-lfs
    inetutils
    efibootmgr
    lm_sensors
    home-manager
  ];
}
